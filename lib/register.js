...
import { TokenStorage, displayQRCode, ContextStorage } from './client.js';
import { SEND_MESSAGE } from './sends.js';

const register = (client, logger, credentials) => {
    ...
    let botId = '';
    let currentCredentials = credentials || null;
    const createUserAvatar = (userId) => {
        return `https://weixin.qq.com/avatar/${userId}`;
    };
    const getMessageText = (itemList) => {
        let text = '';
    ...
    const extractMediaFromMessage = (itemList) => {
        const media = [];
        for (const item of itemList || []) {
            if (item.type === 2) {
                media.push({
    ...
    const initBot = async (storedCredentials) => {
