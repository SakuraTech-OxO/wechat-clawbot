...
const detectMediaType = (fileName, buffer) => {
    let mediaType = 3;
    let itemType = 4;
...
const uploadMedia = async (client, file, toUserId, logger) => {
...
const formatToItemList = async (val, toUserId, client, logger) => {
    const itemList = [];
...
const SEND_MESSAGE = async (client, userId, val, logger) => {
    if (!val || val.length <= 0) {
        return [];
    }
...
export { SEND_MESSAGE, detectMediaType, formatToItemList, getFileBuffer, uploadMedia };
